...
//    @Query(value = "select  fnat.id,fnat.description,fnat.code,case when acrt.id is null then 0 else 1 end as flg_exists from  FinancialAccountType fnat left outer join AccountRelatedType acrt on fnat.id=acrt.financialAccountType.id" +
//            " where fnat.deletedDate is null ")
//    List<FinancialAccountType> findByFinancialAccountTypeListObject();
